...
export function OnboardingModal({ isOpen, onClose, userId, onComplete }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white border border-[#E8E4DE] rounded-2xl p-8 w-full max-w-md shadow-2xl">
        {/* Progress dots */}
        <div className="flex gap-2 justify-center mb-8">
          {steps.map(s => (
            <div key={s.id} className={`h-1.5 rounded-full transition-all duration-300 ${s.id === step ? 'w-8 bg-teal-500' : s.id < step ? 'w-4 bg-teal-700' : 'w-4 bg-[#F0EEE9]'}`} />
          ))}
        </div>
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-[#6B6860]">{current.subtitle}</p>
        </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {STUDENT_LEVELS.map(l => (
              <button
                ...
                className={`p-3 rounded-xl border text-sm font-medium transition-all ${prefs.default_level === l.value ? 'border-teal-500 bg-teal-500/10 text-teal-400' : 'border-[#E8E4DE] text-[#6B6860] hover:border-[#C4C0BA]'}`}
              >
                {l.label}
              </button>
            ))}
        ...
        {step === 2 && (
          <select
            ...
            className="w-full bg-[#F7F6F2] border border-[#E8E4DE] rounded-xl px-4 py-3 text-[#2D2D2D] focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500 text-sm"
          >
          ...
          <div className="space-y-3">
            {GOALS.map(g => (
              <button
                ...
                className={`w-full p-4 rounded-xl border text-left transition-all ${prefs.main_goal === g.value ? 'border-teal-500 bg-teal-500/10' : 'border-[#E8E4DE] hover:border-[#C4C0BA]'}`}
              >
                <div className="font-medium text-sm text-[#2D2D2D]">{g.label}</div>
        ...
        <div className="flex items-center justify-between mt-8">
          <button
            onClick={onClose}
            className="text-sm text-[#8C8880] hover:text-[#6B6860] transition-colors"
          >
            Skip
          </button>
          {step < 3 ? (
