...
export function OnboardingModal({ isOpen, onClose, userId, onComplete }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-[#1E293B] border border-[#334155] rounded-2xl p-8 w-full max-w-md shadow-2xl">
        {/* Progress dots */}
        <div className="flex gap-2 justify-center mb-8">
          {steps.map(s => (
            <div key={s.id} className={`h-1.5 rounded-full transition-all duration-300 ${s.id === step ? 'w-8 bg-teal-500' : s.id < step ? 'w-4 bg-teal-700' : 'w-4 bg-[#334155]'}`} />
          ))}
        </div>
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="text-xl font-bold text-[#F8FAFC] mb-1">{current.title}</h2>
          ...
          <div className="grid grid-cols-2 gap-2">
            {STUDENT_LEVELS.map(l => (
              <button
                ...
                className={`p-3 rounded-xl border text-sm font-medium transition-all ${prefs.default_level === l.value ? 'border-teal-500 bg-teal-500/10 text-teal-400' : 'border-[#334155] text-[#94A3B8] hover:border-[#475569]'}`}
              >
                {l.label}
              </button>
            ))}
        ...
        {step === 2 && (
          <select
            ...
            className="w-full bg-[#0F172A] border border-[#334155] rounded-xl px-4 py-3 text-[#F8FAFC] focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500 text-sm"
          >
          ...
          <div className="space-y-3">
            {GOALS.map(g => (
              <button
                ...
                className={`w-full p-4 rounded-xl border text-left transition-all ${prefs.main_goal === g.value ? 'border-teal-500 bg-teal-500/10' : 'border-[#334155] hover:border-[#475569]'}`}
              >
                <div className="font-medium text-sm text-[#F8FAFC]">{g.label}</div>
                <div className="text-xs text-[#94A3B8] mt-0.5">{g.desc}</div>
              </button>
            ))}
          </div>
        )}
