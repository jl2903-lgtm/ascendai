...
export function ClassSelector({ onClassSelected, selectedId }: ClassSelectorProps) {
  ...
  if (loading) {
    return (
      <div className="h-10 bg-gray-100 rounded-xl animate-pulse w-56" />
    )
  }
  ...
  if (classes.length === 0) {
    return (
      <Link
        ...
        className="inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-dashed border-gray-300 text-sm text-gray-500 hover:border-teal-400 hover:text-teal-600 hover:bg-teal-50/30 transition-all font-semibold"
      >
    ...
    <div className="relative inline-block">
      <button
        ...
        className={cn(
          ...
          selected
            ...
            : 'border-gray-200 bg-white text-gray-600 hover:border-teal-400 hover:text-teal-600'
        )}
      >
      ...
      {open && (
        <>
          ...
          <div className="absolute left-0 top-full mt-1 z-20 bg-white border border-gray-200 rounded-xl shadow-lg min-w-[220px] py-1 overflow-hidden">
            {selected && (
              <button
                onClick={() => handleSelect(null)}
                className="w-full px-4 py-2.5 text-left text-sm text-gray-500 hover:bg-gray-50 transition-colors font-medium"
              >
                Clear selection
              </button>
            )}
            {classes.map(profile => (
              ...
              >
                ...
                <div className="text-xs text-gray-400 font-medium mt-0.5">{profile.cefr_level} · {profile.course_type}</div>
              </button>
            ))}
