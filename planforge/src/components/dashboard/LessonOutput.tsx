...
export function LessonOutput({ lesson, formData, onAdjust, adjusting }: Props) {
  ...
  return (
    <div className="space-y-4">
      {/* Title card */}
      <div className="bg-gradient-to-br from-teal-600/20 to-[#1E293B] border border-teal-600/40 rounded-2xl p-6">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <h2 className="text-xl font-bold text-[#F8FAFC]">{lesson.title}</h2>
            <div className="flex flex-wrap gap-2 mt-2">
              {[
                ...
                <span key={text} className="flex items-center gap-1.5 bg-[#0F172A]/60 border border-[#334155] rounded-lg px-2.5 py-1 text-xs text-[#94A3B8]">
      ...
      <div className="bg-[#1E293B] border border-[#334155] rounded-2xl p-4">
        <div className="text-xs text-[#94A3B8] font-medium mb-3 uppercase tracking-wider">Adjust Lesson</div>
        <div className="flex flex-wrap gap-2">
          {[
            ...
            <button
              ...
              disabled={adjusting}
              className="flex items-center gap-1.5 text-xs border border-[#334155] hover:border-teal-500 text-[#94A3B8] hover:text-teal-400 px-3 py-1.5 rounded-lg transition-all disabled:opacity-40"
            >
      ...
      <div className="bg-[#1E293B] border border-[#334155] rounded-2xl overflow-hidden">
        <div className="flex overflow-x-auto border-b border-[#334155] px-2 pt-2 gap-1 scrollbar-hide">
          {sections.map(s => (
            <button
              key={s.key}
              onClick={() => {
              ...
              className={`flex items-center gap-1.5 px-3 py-2 text-xs font-medium rounded-t-lg whitespace-nowrap transition-all flex-shrink-0 ${activeSection === s.key ? 'bg-[#0F172A] text-teal-400 border-t border-x border-[#334155]' : 'text-[#475569] hover:text-[#94A3B8]'}`}
            >
        ...
        <div className="p-6 space-y-8 max-h-[70vh] overflow-y-auto">
          ...
          <div id="section-warmer" className="scroll-mt-4">
            ...
            <div className="mt-4 space-y-3">
              <p className="text-sm text-[#CBD5E1] leading-relaxed">{lesson.warmer.instructions}</p>
          ...
          <div id="section-leadIn" className="scroll-mt-4">
            ...
            <div className="mt-4 space-y-3">
              <p className="text-sm text-[#CBD5E1] leading-relaxed">{lesson.leadIn.instructions}</p>
              {lesson.leadIn.context && <p className="text-sm text-[#94A3B8] italic leading-relaxed">{lesson.leadIn.context}</p>}
            </div>
          </div>
          ...
          <div id="section-mainActivity" className="scroll-mt-4">
            ...
            <div className="mt-4 space-y-3">
              <p className="text-sm text-[#CBD5E1] leading-relaxed">{lesson.mainActivity.instructions}</p>
              {lesson.mainActivity.variations && (
                <div className="bg-blue-500/5 border border-blue-500/20 rounded-xl p-3">
                  <div className="text-xs font-semibold text-blue-400 mb-1">Variations</div>
                  <p className="text-sm text-[#94A3B8] leading-relaxed">{lesson.mainActivity.variations}</p>
                </div>
              )}
          ...
          <div id="section-languageFocus" className="scroll-mt-4">
            ...
            <div className="mt-4 space-y-4">
              <div>
                ...
                <p className="text-sm text-[#CBD5E1] leading-relaxed">{lesson.languageFocus.explanation}</p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <InfoCard title="Examples">
                  <ul className="space-y-1">
                    {lesson.languageFocus.examples.map((e, i) => (
                      <li key={i} className="text-sm text-[#CBD5E1] font-mono bg-[#0F172A] px-2 py-1 rounded">{e}</li>
                    ))}
                  </ul>
                </InfoCard>
          ...
          <div id="section-exercises" className="scroll-mt-4">
            ...
            <div className="mt-4 space-y-4">
              {lesson.exercises.map((ex, i) => (
                <div key={i} className="bg-[#0F172A] border border-[#334155] rounded-xl p-4">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-sm font-semibold text-[#F8FAFC]">{ex.type}</span>
                  </div>
                  <p className="text-sm text-[#94A3B8] mb-3">{ex.instructions}</p>
                  <p className="text-sm text-[#CBD5E1] whitespace-pre-wrap leading-relaxed">{ex.content}</p>
                  {ex.answerKey && (
                    <div className="mt-3 pt-3 border-t border-[#334155]">
                      <div className="text-xs font-semibold text-teal-400 mb-1">Answer Key</div>
                      <p className="text-xs text-[#94A3B8]">{ex.answerKey}</p>
                    </div>
                  )}
                </div>
              ))}
          ...
          <div id="section-speakingTask" className="scroll-mt-4">
            ...
            <div className="mt-4 space-y-3">
              <p className="text-sm text-[#CBD5E1] leading-relaxed">{lesson.speakingTask.instructions}</p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {lesson.speakingTask.prompts.map((p, i) => (
                  <div key={i} className="bg-[#0F172A] border border-[#334155] rounded-lg px-3 py-2 text-sm text-[#CBD5E1] flex items-start gap-2">
          ...
          <div id="section-exitTicket" className="scroll-mt-4">
            ...
            <div className="mt-4 space-y-3">
              <p className="text-sm text-[#94A3B8]">{lesson.exitTicket.instructions}</p>
              <div className="space-y-2">
                {lesson.exitTicket.questions.map((q, i) => (
                  <div key={i} className="flex items-start gap-3 bg-[#0F172A] border border-[#334155] rounded-lg px-4 py-3">
                    <span className="text-teal-400 font-semibold text-sm flex-shrink-0">Q{i + 1}</span>
                    <p className="text-sm text-[#CBD5E1]">{q}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {lesson.homework.instructions && (
            <div id="section-homework" className="scroll-mt-4">
              ...
              <div className="mt-3 bg-[#0F172A] border border-[#334155] rounded-xl p-4">
                <p className="text-sm text-[#CBD5E1] leading-relaxed">{lesson.homework.instructions}</p>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="bg-[#1E293B] border border-[#334155] rounded-2xl p-4 flex flex-wrap gap-3">
        ...
        <button
          ...
          className="flex items-center gap-2 text-sm font-medium px-4 py-2.5 rounded-xl border border-[#334155] hover:border-teal-500 text-[#94A3B8] hover:text-white transition-all"
        >
...
function SectionHeader({ icon: Icon, title, timing, badge }: { icon: React.FC<{ className?: string }>; title: string; timing?: string; badge?: string }) {
  return (
    <div className="flex items-center gap-2 pb-3 border-b border-[#334155]">
      ...
      {timing && (
        <span className="flex items-center gap-1 text-xs text-[#94A3B8] bg-[#0F172A] px-2 py-0.5 rounded-full border border-[#334155]">
...
function InfoCard({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="bg-[#0F172A] border border-[#334155] rounded-xl p-4">
      <div className="text-xs font-semibold text-[#94A3B8] uppercase tracking-wider mb-3">{title}</div>
      {children}
    </div>
  )
}
