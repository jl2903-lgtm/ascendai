...
export async function generateLessonPDF(lesson: LessonContent, meta: { level: string; topic: string; date: string }) {
  ...
  doc.setFont('helvetica', 'bold')
  doc.text('PlanForge', margin, 16)
  doc.setFontSize(10)
  doc.setTextColor(148, 163, 184)
  doc.setFont('helvetica', 'normal')
  doc.text('AI-Powered ESL Lesson Plan', margin, 23)
  y = 38
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text(`PlanForge | ${meta.level} | ${meta.topic} | ${meta.date}`, margin, doc.internal.pageSize.getHeight() - 5)
    doc.text(`Page ${i} of ${pageCount}`, pageWidth - margin, doc.internal.pageSize.getHeight() - 5, { align: 'right' })
  }

  doc.save(`planforge-lesson-${Date.now()}.pdf`)
}
...
export async function generateWorksheetPDF(worksheet: WorksheetContent, date: string) {
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.setFontSize(8)
    doc.setTextColor(100, 116, 139)
    doc.text(`PlanForge Worksheet | ${worksheet.level} | ${date}`, margin, doc.internal.pageSize.getHeight() - 4)
    doc.text(`Page ${i} of ${pageCount}`, pageWidth - margin, doc.internal.pageSize.getHeight() - 4, { align: 'right' })
  }

  doc.save(`planforge-worksheet-${Date.now()}.pdf`)
}
