      ...
      <div className="min-h-screen bg-[#F7F6F2] flex items-center justify-center px-6">
        <div className="w-full max-w-sm">
          <div className="text-center mb-8">
            <div className="w-12 h-12 bg-white border border-[#E8E4DE] rounded-xl flex items-center justify-center mx-auto mb-4">
              <Lock className="w-6 h-6 text-[#6B6860]" />
            </div>
            <h1 className="text-xl font-bold text-[#2D2D2D]">Admin Access</h1>
          </div>
          <form onSubmit={login} className="bg-white border border-[#E8E4DE] rounded-2xl p-6 space-y-4">
            {error && <div className="bg-red-500/10 border border-red-500/30 rounded-lg px-4 py-3 text-red-400 text-sm">{error}</div>}
            <div>
              <label className="block text-sm font-medium text-[#6B6860] mb-2">Admin Password</label>
              <input
                type="password"
                value={password}
                onChange={e => setPassword(e.target.value)}
                className="w-full bg-[#F7F6F2] border border-[#E8E4DE] rounded-xl px-4 py-3 text-[#2D2D2D] focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500"
              />
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              ...
              const colorMap: Record<string, string> = {
                ...
                slate: 'text-[#8C8880] bg-[#F0EEE9]',
              }
              return (
                <div key={stat.label} className="bg-white border border-[#E8E4DE] rounded-2xl p-6">
                  ...
                  <div className="text-sm font-medium text-[#6B6860]">{stat.label}</div>
                  <div className="text-xs text-[#8C8880] mt-1">{stat.sub}</div>
                </div>
              )
            })}
          </div>
