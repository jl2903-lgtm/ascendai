  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-[#F8FAFC]">Lesson Generator</h1>
          <p className="text-sm text-[#94A3B8]">Complete, communicative lesson plans in 60 seconds</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2">
          <div className="bg-[#1E293B] border border-[#334155] rounded-2xl p-6 space-y-5 sticky top-6">
            <h2 className="text-sm font-semibold text-[#94A3B8] uppercase tracking-wider">Class Details</h2>
            ...
            {/* Topic */}
            <div>
              <label className="block text-sm font-medium text-[#94A3B8] mb-2">Topic or Skill Focus <span className="text-red-400">*</span></label>
              <input
                type="text"
                value={form.topic}
                ...
                className={`w-full bg-[#0F172A] border rounded-xl px-4 py-2.5 text-sm text-[#F8FAFC] placeholder-[#475569] focus:outline-none focus:ring-1 transition-colors ${errors.topic ? 'border-red-500 focus:ring-red-500' : 'border-[#334155] focus:border-teal-500 focus:ring-teal-500'}`}
              />
            ...
            {/* Length */}
            <div>
              <label className="block text-sm font-medium text-[#94A3B8] mb-2">Lesson Length</label>
              <div className="grid grid-cols-4 gap-1.5">
                {LESSON_LENGTHS.map(l => (
                  <button
                    key={l.value}
                    onClick={() => setForm(f => ({ ...f, length: l.value }))}
                    className={`py-2 rounded-lg text-xs font-medium transition-all ${form.length === l.value ? 'bg-teal-600 text-white' : 'bg-[#0F172A] border border-[#334155] text-[#94A3B8] hover:border-[#475569]'}`}
                  >
                    {l.value}m
                  </button>
                ))}
            ...
            {/* Age group */}
            <div>
              <label className="block text-sm font-medium text-[#94A3B8] mb-2">Student Age Group</label>
              <div className="relative">
                <select
                  value={form.ageGroup}
            ...
            {/* Nationality */}
            <div>
              <label className="block text-sm font-medium text-[#94A3B8] mb-2">Student Nationality / L1</label>
              <div className="relative">
                <select
            ...
            {/* Class size */}
            <div>
              <label className="block text-sm font-medium text-[#94A3B8] mb-2">Class Size</label>
              <div className="relative">
                <select
            ...
            {/* Special focus */}
            <div>
              <label className="block text-sm font-medium text-[#94A3B8] mb-2">Special Focus <span className="text-[#475569] font-normal">(optional)</span></label>
              <div className="flex flex-wrap gap-2">
                {SPECIAL_FOCUS_OPTIONS.map(o => (
                  <button
                    key={o.value}
                    onClick={() => toggleFocus(o.value)}
                    className={`text-xs px-3 py-1.5 rounded-lg border transition-all ${form.specialFocus.includes(o.value) ? 'border-teal-500 bg-teal-500/10 text-teal-400' : 'border-[#334155] text-[#94A3B8] hover:border-[#475569]'}`}
                  >
                    {o.label}
                  </button>
                ))}
        ...
        <div className="lg:col-span-3 min-h-[400px]">
          {loading ? (
            <div className="bg-[#1E293B] border border-[#334155] rounded-2xl h-full min-h-[500px] flex items-center justify-center">
              <ThinkingLoader />
            </div>
          ) : lesson ? (
            <LessonOutput
            ...
            <div className="bg-[#1E293B] border border-[#334155] rounded-2xl h-full min-h-[400px] flex flex-col items-center justify-center text-center p-8">
              ...
              <h3 className="text-lg font-semibold text-[#F8FAFC] mb-2">Your lesson will appear here</h3>
              <p className="text-sm text-[#94A3B8] max-w-xs">Fill in your class details and click Generate Lesson to create a complete, communicative lesson plan in seconds.</p>
              <div className="mt-6 grid grid-cols-2 gap-2 w-full max-w-xs">
                {['Warmer Activity', 'Language Focus', 'Main Task', 'Exit Ticket'].map(s => (
                  <div key={s} className="bg-[#0F172A] rounded-xl p-3 text-xs text-[#475569] flex items-center gap-2">
