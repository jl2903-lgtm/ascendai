  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-[#F8FAFC]">Demo Lesson Builder</h1>
          <p className="text-sm text-[#94A3B8]">Interview-ready lesson plans that impress hiring panels</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2">
          <div className="bg-[#1E293B] border border-[#334155] rounded-2xl p-6 space-y-5 sticky top-6">
            <h2 className="text-sm font-semibold text-[#94A3B8] uppercase tracking-wider">Interview Details</h2>

            <Field label="Target School Type">
              <div className="relative">
                <select value={form.schoolType} onChange={e => setForm(f => ({ ...f, schoolType: e.target.value }))} className="w-full appearance-none bg-[#0F172A] border border-[#334155] rounded-xl px-4 py-2.5 text-sm text-[#F8FAFC] focus:outline-none focus:border-teal-500 pr-9">
            ...
            <Field label="Student Level">
              <div className="relative">
                <select value={form.level} onChange={e => setForm(f => ({ ...f, level: e.target.value }))} className="w-full appearance-none bg-[#0F172A] border border-[#334155] rounded-xl px-4 py-2.5 text-sm text-[#F8FAFC] focus:outline-none focus:border-teal-500 pr-9">
              ...
              <div className="grid grid-cols-3 gap-1.5">
                {DEMO_LENGTHS.map(d => (
                  <button
                    ...
                    className={`py-2 rounded-lg text-xs font-semibold transition-all ${form.demoLength === d.value ? 'bg-amber-600 text-white' : 'bg-[#0F172A] border border-[#334155] text-[#94A3B8] hover:border-[#475569]'}`}
                  >
                    {d.value}m
                  </button>
                ))}
            ...
            <Field label="Your Experience Level">
              <div className="relative">
                <select value={form.experienceLevel} onChange={e => setForm(f => ({ ...f, experienceLevel: e.target.value }))} className="w-full appearance-none bg-[#0F172A] border border-[#334155] rounded-xl px-4 py-2.5 text-sm text-[#F8FAFC] focus:outline-none focus:border-teal-500 pr-9">
        ...
        <div className="lg:col-span-3">
          {loading ? (
            ...
            <div className="space-y-4">
              {/* Title */}
              <div className="bg-gradient-to-br from-amber-600/20 to-[#1E293B] border border-amber-600/40 rounded-2xl p-6">
                <h2 className="text-xl font-bold text-[#F8FAFC]">{result.title}</h2>
                <div className="flex flex-wrap gap-2 mt-3">
                  {[result.targetSchool, result.overview.level, result.overview.duration].map(t => (
                    <span key={t} className="text-xs px-2.5 py-1 bg-[#0F172A]/60 border border-[#334155] rounded-lg text-[#94A3B8]">{t}</span>
                  ))}
                </div>
                <div className="mt-3">
                  <div className="text-xs font-semibold text-[#94A3B8] mb-2">Learning Objectives</div>
              ...
              <div className="space-y-4">
                {result.stages.map((stage, i) => (
                  <div key={i} className="bg-[#1E293B] border border-[#334155] rounded-2xl overflow-hidden">
                    <div className="bg-[#0F172A] px-5 py-3 flex items-center justify-between border-b border-[#334155]">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-semibold text-[#F8FAFC] text-sm">{stage.name}</span>
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-[#334155]">
                      <div className="p-5">
                        <div className="text-xs font-semibold text-[#94A3B8] uppercase tracking-wider mb-2">Activities</div>
                        <p className="text-sm text-[#CBD5E1] leading-relaxed">{stage.activities}</p>
                      </div>
                      <div className="p-5 bg-amber-500/5">
                        ...
                        <p className="text-sm text-[#94A3B8] leading-relaxed italic">{stage.whyItWorks}</p>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              {result.interviewTips?.length > 0 && (
                <div className="bg-[#1E293B] border border-[#334155] rounded-2xl p-5">
                  <h3 className="font-semibold text-[#F8FAFC] mb-3">Interview Tips</h3>
            ...
            <div className="bg-[#1E293B] border border-[#334155] rounded-2xl min-h-[400px] flex flex-col items-center justify-center text-center p-8">
              ...
              <h3 className="text-lg font-semibold text-[#F8FAFC] mb-2">Build your interview lesson</h3>
              <p className="text-sm text-[#94A3B8] max-w-xs">Every stage comes with a methodology explanation to help you speak confidently in your interview. Get the job.</p>
            </div>
          )}
        </div>
      </div>
